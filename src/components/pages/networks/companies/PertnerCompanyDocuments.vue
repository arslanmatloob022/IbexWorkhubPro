...
const get_comapny_documents = async () => {
  try {
    ...
    const response = await api.get(
      `v3/api/partner/company_document/${props.id}/get_documents_of_a_company/`,
      {}
    );
...
const getCompanyIndustryDocs = async () => {
  try {
    const resp = await api.get(
      `/v3/api/partner/compliance/template/get-company-indutries-tmeplates/${props.id}/`
    );
...
const AssignTemToIdustry = async () => {
  try {
    ...
    const resp = await api.post(
      "/v3/api/partner/compliance/template/assign-template-to-company/",
      {
        company: props.id,
...
const deleteCompanyDocHandler = async () => {
  try {
    SweetAlertProps.value.isSweetAlertOpen = false;
    const response = await api.delete(
      `/v3/api/partner/company_document/${currentSelectedID.value}/`,
      {}
    );
    get_comapny_documents();
    notyf.dismissAll();
...
const archiveCompanyDoc = async () => {
  try {
    ...
    const response = await api.patch(
      `/v3/api/partner/company_document/${currentSelectedID.value}/archive-company-document/`,
      {
