...
const getcompanydocById = async (id: any) => {
    Loading.value = true
    const response = await api.get(`v3/api/partner/company_document/${id}/`, {})
...
const AddCompanydocHandler = async () => {
    try {
        ...
        if (props.docID) {
            const response = await api.patch(`/v3/api/partner/company_document/${props.docID}/`, formDataAPI)
        }
        else {
            formDataAPI.append('industry', props.industry)
            formDataAPI.append('arezCompany', company.loggedCompany.id)
            const response = await api.post(`/v3/api/partner/company_document/`, formDataAPI)
        }
