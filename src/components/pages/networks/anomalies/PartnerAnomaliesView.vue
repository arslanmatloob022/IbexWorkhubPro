...
let sessoionPartnerAnomalieFilters = <any>{};
...
const statChangeHandler = (anomalyType: string) => {
  ...
  sessoionPartnerAnomalieFilters.currentStatType = anomalyType;
  window.sessionStorage.setItem(
    `partneranomalieFilters${company.loggedCompany.id}`,
    JSON.stringify(sessoionPartnerAnomalieFilters)
  );
};
...
const getAnomaliesData = async (
  ...
  try {
    ...
    sessoionPartnerAnomalieFilters.industry = selectedOptions.value;
    sessoionPartnerAnomalieFilters.documents = selectedDocList.value;
    sessoionPartnerAnomalieFilters.limitedScreening = limitedScreening.value;
    window.sessionStorage.setItem(
      `partneranomalieFilters${company.loggedCompany.id}`,
      JSON.stringify(sessoionPartnerAnomalieFilters)
    );
    ...
    const response = await api.get(
      `/v3/api/partner/anomalies/anomales-data/${
    ...
    router.push({ query: { ...route.query, page: 1 } });
    if (sessoionPartnerAnomalieFilters.currentStatType) {
      statChangeHandler(sessoionPartnerAnomalieFilters.currentStatType);
    }
...
const debouncedGetAnomaliesData = debounce(() => {
  if (
    isSelectedDocFirstCall &&
    sessoionPartnerAnomalieFilters.documents.length
  ) {
...
// watch(docTypeSelected, (newVal, oldVal) => {
//     if (newVal.length > 1) {
//         docTypeSelected.value = [newVal[newVal.length - 1]];
//     }
...
onMounted(() => {
  statChangeHandler("all");
  Loading.value = true;
  sessoionPartnerAnomalieFilters = JSON.parse(
  ...
  console.log("sesssion value ", sessoionPartnerAnomalieFilters);
  if (sessoionPartnerAnomalieFilters.limitedScreening) {
    limitedScreening.value = true;
  }
  if (sessoionPartnerAnomalieFilters.documents) {
    console.log("inside documents list");
    selectedDocList.value = sessoionPartnerAnomalieFilters.documents;
  }
  if (sessoionPartnerAnomalieFilters.industry.length) {
    selectedOptions.value = sessoionPartnerAnomalieFilters.industry;
  } else {
    selectedOptions.value = company.loggedCompany.industry;
  }
});
        ...
        <div style="position: relative" class="columns is-multilines">
          ...
          <div class="column is-2 m-1">
            <div class="p-2" style="max-height: 280px">
              ...
              <!-- <VField>
                                <VControl>
                                    <VSwitchBlock v-model="docAllNoneFilter" label="All" thin />
                                </VControl>
                            </VField> -->
              <div
              ...
              >
                <div class="column is-12">
                  <div class="cu-pointer">
                    <!-- <VField horizontal>
                                            <VControl>
                                              <VSwitchSegment
                                                v-model="docAllNoneFilter"
                                                label-true="All"
                                                label-false="None"
                                                color="primary"
                                              />
                    ...
                    <!-- <div class="account-menu">
                                            ...
                                            <div @click="getAnomaliesData()" class="">
                                                <span class="button-image">
                                                    <img src="/images/allicons8.png" alt="Button Image">
                                                </span>
                                                <span style="font-size:13px;color:#666,"
                                                    :style="{ color: currentTabId === 0 ? 'var(--primary)' : '' }">All
                                                </span>
                                            </div>
                                        </div> -->
                  </div>
                    ...
                    <div class="industries p-1">
                      ...
                      >
                        <VLabel tabindex="0" class="tool-card cus-align-center">
                          ...
                          />
                          <div class="tool-card-inner cus-align-center">
                            <!-- <img style="width:16px" :src="doc.icon" /> -->
                            <span
          ...
          <div class="column is-8">
            <div class="p-0">
              <div class="is-flex space-between">
                <div class="cus-align-center">
                  <h1 class="title is-5 toc-ignore mb-0">
                    <!-- {{
                                        selectedDocList.length == anomaliesOptions.length ? `All
                                        (${selectedDocList.length})` : `Documents (${selectedDocList.length})` }} -->
                  </h1>
                  <!-- <i aria-hidden="true" class="fas fa-arrow-right light-text" /> -->
                </div>
                <div class="cus-align-center">
              ...
              >
                <div v-for="item in selectedDocList">
                  <!-- <span style="color: var(--info);"> #{{ item.documentTitle }}</span> -->
                  <VTag
            ...
            <div class="cu-card-container">
              ...
              >
                ...
                <img
                  ...
                  @click="
                    () => {
                      $router.push(`/sidebar/company/supplier/${item.id}`);
                    }
                  "
                  :src="
                ...
                >
                  <li class="data-list-item" v-if="index < 2">
                    ...
                    <!-- <span>
                                            ...
                                            :src="anomaly.approved ? '/images/iconapprove.png' : '/images/disapproved.png'" />
                                            </span> -->
                    <VTag
